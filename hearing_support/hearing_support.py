...
def audio_callback(indata, frames, time_info, status):
    ...
    audio_q.put(mono_data)                          # 認識用にキューへ追加
...
def recognize_worker():
    while not st.session_state.stop_flag.is_set():      # 停止フラグが立つまでループ
        try:
            data = audio_q.get(timeout=1)           # 音声データを取得（1秒待機）
...
# ▶️ 「再開」ボタンが押されたときの処理
if st.button("🎙️ 再開"):
    # 停止フラグが立っている（＝録音・認識が止まっている）場合のみ再開
    if st.session_state.stop_flag.is_set():
        st.session_state.stop_flag.clear()  # 停止フラグを解除（録音・認識を再開可能に）

        # 音声認識処理を別スレッドで再起動（バックグラウンドで動作）
        threading.Thread(target=recognize_worker, daemon=True).start()

        # 音声入力ストリーム（録音処理）を別スレッドで再起動
        threading.Thread(target=start_stream, daemon=True).start()
        # 再開ボタンの中に以下を追加：
        st.session_state.thread_started = True
